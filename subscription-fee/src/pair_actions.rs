...
pub trait PairActionsModule: crate::common_storage::CommonStorageModule {
    ...
    fn add_pair_address(&self, payment_token_id: TokenIdentifier, pair_address: ManagedAddress) {
        require!(
            payment_token_id.is_valid_dct_identifier(),
            "Invalid token ID"
        );
        require!(
    ...
    fn get_worth_of_price(
        ...
        let stable_pair_data_mapper = self.pair_address_for_token(&wmoax_token_id);
        if stable_pair_data_mapper.is_empty() {
            return Result::Err(());
        }
        let stable_pair_address = stable_pair_data_mapper.get();
        let wmoax_price: DctTokenPayment = self
            .pair_proxy(price_query_address.clone())
            .get_safe_price_by_default_offset(
                stable_pair_address,
                DctTokenPayment::new(stable_token_id, 0, stable_worth_amount),
            )
            .execute_on_dest_context();

        if desired_token_id == &wmoax_price.token_identifier {
            return Result::Ok(wmoax_price.amount);
        }
        ...
        let price: DctTokenPayment = self
            ...
            .get_safe_price_by_default_offset(pair_address, wmoax_price)
            .execute_on_dest_context();
