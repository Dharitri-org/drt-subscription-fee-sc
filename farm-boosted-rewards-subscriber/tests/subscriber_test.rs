...
static WMOAX_TOKEN_ID: &[u8] = b"WMOAX-123456";
...
fn claim_boosted_rewards_for_user_test() {
    ...
    b_mock_rc
        .borrow_mut()
        .set_dct_balance(&user, WMOAX_TOKEN_ID, &rust_biguint!(1_000_000));

    subscription_setup
        .call_deposit(&user, WMOAX_TOKEN_ID, 1_000_000)
        .assert_ok();
    ...
    b_mock_rc
        .borrow_mut()
        .set_dct_balance(&user, LP_TOKEN_ID, &rust_biguint!(user_token_amount));
...
fn claim_boosted_rewards_for_user_multiple_farms_test() {
    ...
    // Generate farm rewards
    let user_token_amount = 100_000_000;
    b_mock_rc.borrow_mut().set_dct_balance(
        &user,
        LP_TOKEN_ID,
...
fn mex_operation_with_claim_fees_test() {
    ...
    b_mock_rc
        .borrow_mut()
        .set_dct_balance(&first_user, WMOAX_TOKEN_ID, &rust_biguint!(1_000_000));

    subscription_setup
        .call_deposit(&first_user, WMOAX_TOKEN_ID, 1_000_000)
        .assert_ok();
    ...
    subscription_setup
        .call_deposit(&second_user, WMOAX_TOKEN_ID, 1_000_000)
        .assert_ok();
...
fn subtract_worth_of_stable_payment_test() {
    ...
    // Pool ratio is 1:2, so for a service payment of 500 USDC worth of WMOAX, the amount should be 250
    subscriber_setup
        .call_subtract_payment(vec![user_id])
        .assert_ok();
